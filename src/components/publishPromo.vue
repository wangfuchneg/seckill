...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      // 拼接完整时间字符串（传给后端）
      const startDateTime = `${this.formData.Date1} ${this.formData.startTime}`;
      const endDateTime = `${this.formData.Date2} ${this.formData.endTime}`;
      if (!this.formData.price || !this.formData.promoItemPrice) {
        this.message = "定价和现价不能为空";
        this.error = true;
        this.success = false;
        return;
      }
      ...
      const itemData = {
        ...
        pageKeywords: this.formData.pageKeywords,
        imgUrl: this.imgUrl,
        promoName: this.formData.promoName, // 新增：传递促销活动名称
      };
      this.isSubmitting = true;
      // 直接使用固定的完整接口地址，无需组装
      axios
          .post('http://localhost:8082/publishPromo', itemData)
          .then((response) => {
            this.message = "商品信息提交成功";
            this.success = true;
            this.error = false;
            console.log("接口调用成功，返回结果：", response.data);
          })
          .catch((error) => {
            this.message = `商品信息提交失败: ${error.response?.data?.message || error.message}`;
            this.error = true;
            this.success = false;
